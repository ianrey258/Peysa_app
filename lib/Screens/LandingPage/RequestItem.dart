...
class RequestItemstate extends State<RequestItem>{
  ...
  Widget getImage(image) {
    return Container(
      width: 75,
      child: image.isNotEmpty ? Image.network(ImageController.getItemNetImage(image['filename']),filterQuality: FilterQuality.medium,fit: BoxFit.cover) : 
  ...
  Widget requestItem(item,status,image){
    return ListTile(
      ...
      onTap: (){Navigator.pushNamed(context, 'Bidding',arguments: BidItem(id: item['id'],bidItem: item['bidItem']));},
    );
  }

  Widget requestedItem(otherBidItem,bidItem,itemDetail,itemImage,bidManager,userDetail){
    ...
    return ListTile(
      ...
      title: Row(
        ...
        children: <Widget>[
          Align(
            alignment: Alignment.bottomLeft,
            child: Text(userDetail['firstname']+' '+userDetail['lastname'],style: TextStyle(fontWeight: otherBidItem['requestStatus'] == '12'? FontWeight.bold : FontWeight.normal))
          ),
          Align(
            alignment: Alignment.bottomRight,
            child: Text("Date: "+bidItem['datetime'],style: TextStyle(fontSize: 13),)
          )
        ],
      ),
      subtitle: Row(
        ...
        children: <Widget>[
          Align(
            alignment: Alignment.bottomLeft,
            child: Text("Budget P"+itemDetail['itemPriceorBudget'],style: TextStyle(fontWeight: otherBidItem['requestStatus'] == '12'? FontWeight.bold : FontWeight.normal),)
          ),
          Align(
            alignment: Alignment.bottomRight,
            child: Text("Status: "+status,style: TextStyle(fontSize: 13),)
          )
        ],
      ),
  ...
  Future<bool> requestFeedbackDialog(otherBidItem,itemDetail,userDetail){
    return showDialog(
      context: context,
      builder: (_) => RequestFeedback(otherBidItem: otherBidItem,itemDetail: itemDetail,userDetail: userDetail),
    );
  }
      ...
      child: FutureBuilder(
        ...
        builder: (_,data){
          if(data.connectionState == ConnectionState.done){
            if(data.hasData){
              if(!data.data[3]) return Container(child: Center(child: Text('Check Internet Connection'),),);
              if(data.data[0].isNotEmpty ){
                ...
                return Container(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_,index){
                      return Container(
                        child: requestItem(data.data[0][length-index],data.data[1][length-index],data.data[2][length-index]),
                      );
                    }
                  ),
                );
              ...
              return Container(
                child: Center(child: Text('You Have no Request Item'),),
              );
            }
          }
    ...
    return RefreshIndicator(
      child: FutureBuilder(
        ...
        builder: (_,data){
          if(data.connectionState == ConnectionState.done){
            if(data.hasData){
              if(!data.data[6]) return Container(child: Center(child: Text('Check Internet Connection'),),);
              if(data.data[0].isNotEmpty){
                int length = data.data[0].length - 1;
                return Container(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_,index){
                      return Container(
                        child: requestedItem(data.data[0][length-index],data.data[1][length-index],data.data[2][length-index],data.data[3][length-index],data.data[4][length-index],data.data[5][length-index]),
                      );
                    }
                  ),
                );
              ...
              return Container(
                  child: Center(child: Text('You Have no Others Request Yet'),),
              ); 
            }
          }
          return Container(child: Center(child: Text('Loading .....'),),);
...
class _RequestFormState extends State<RequestForm> {
  ...
  validate() async {
    if(_key.currentState.validate()){
      ...
      await DataController.getAddresses().then((value) => value.forEach((key, value) {if(value['location'] == address){text[3].text = value['id'];}}));
      LoadingScreen.showLoading(context, 'Sending to '+address);
      var result = await DataController.sendingRequest(images, text);
      Navigator.pop(context);
      try{
        await LoadingScreen.showResultDialog(context, int.parse(result).toString()+' User Sended', MediaQuery.of(context).textScaleFactor*25);
        Navigator.pop(context);
      } catch(e){
        await LoadingScreen.showResultDialog(context, result, MediaQuery.of(context).textScaleFactor*25);
      }
    }
  }
    ...
    return FutureBuilder<Map>(
      ...
      builder: (context,snapshot){
        if(snapshot.connectionState == ConnectionState.done){
          if(snapshot.hasData){
            if(address == ''){address = snapshot.data.values.toList().map((e) => e['location']).toList()[0].toString();}
            return Container(
  ...
  _getPicture(source) async {
    ...
    try{
      ...
      setState(() {
        _item != null && _item.lengthSync()<5000000 ? images.add(_item) : invalidImageSize();
      });
    }catch(e){}
    if(_item != null && _item.lengthSync()<5000000 || _item == null) Navigator.pop(context);
  }
  ...
  List<Widget> getImages(){
    return images.isEmpty ? [Center(child: Text("No Sample Image"))] : images.map((e) => sampleImage(e)).toList();
  }
...
class RequestFeedback extends StatefulWidget {
  ...
  RequestFeedback({Key key, this.otherBidItem,this.itemDetail,this.userDetail}):super(key: key);
  @override
...
class _RequestFeedbackState extends State<RequestFeedback> {
  ...
  initAddresses() async {
    ...
    addresses.forEach((key, value) {if(value['id'] == widget.userDetail['zipCode']){address= value['location'];}});
    refreshState();
  }
  ...
  validateAcceptUpdate() async {
    if(_key.currentState.validate()){
      _key.currentState.save();
      BidItemDetail itemdetail = BidItemDetail(id: widget.otherBidItem['suggestedItemId'],itemName: text[0].text,itemPriceorBudget: text[2].text,description: text[1].text);
      LoadingScreen.showLoading(context, 'Loading ...');
      String result = widget.itemDetail['id'] == bidder.suggestedItemId ? await DataController.requestAcceptUpdate(widget.otherBidItem['id'], itemdetail, images) : await DataController.requestAcceptUpdate(widget.otherBidItem['id'], itemdetail, images,'update');
      if(result !=  null){
        Navigator.pop(context);
        await LoadingScreen.showResultDialog(context, result, 25);
        if(widget.itemDetail['id'] != bidder.suggestedItemId) Navigator.pop(context);
      } else {
        Navigator.pop(context);
        await LoadingScreen.showResultDialog(context, "Check Internet Connection", 25);
      }
    }
  }

  requestDecline() async {
    String result  = await DataController.requestDecline(widget.otherBidItem['id']);
    if(result !=  null){
        await LoadingScreen.showResultDialog(context, result, 25);
        Navigator.popAndPushNamed(context, 'RequestItem');
      } else {
        await LoadingScreen.showResultDialog(context, "Cannot Decline due to Check Internet Connectivity", 10);
      }
  }
  ...
  Widget sampleImage(image,[isArray = false,owner = 'sender']){
    return FlatButton(
      padding: EdgeInsets.only(right: 5),
      onPressed: (){imageFullSize(image,isArray,owner);}, 
      child: !isArray ? Image.file(image) : Image.network(ImageController.getItemNetImage(image),fit: BoxFit.contain)
    ); 
  }

  List<Widget> getImages(){
    return images.isEmpty && sampleImages.isEmpty ? [Center(child: Text("No Sample Image"))] : images.map((e) => sampleImage(e,false,'reciever')).toList();
  }
  ...
  deleteImageFromDb(filename) async {
    sampleImagesMap.forEach((key, value) async {if(value['filename'] == filename) await ImageController.removeBidImage(value);});
    refreshState();
    Navigator.pop(context);
  }
    ...
    return showDialog(
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            children: <Widget>[
              owner == 'reciever' ? Align(
                alignment: Alignment.centerRight,
                child: RaisedButton(
                  onPressed: (){
                    !isArray?deleteImage(image):deleteImageFromDb(image);
                    refreshState();
                  },
                  child: Text('Remove'),
                ),
      ...
      builder: (_,snapshot){
        if(snapshot.connectionState == ConnectionState.done){
          if(snapshot.hasData){
            ...
            if(snapshot.data.isNotEmpty && id != widget.itemDetail['id']) snapshot.data.forEach((key, value) {sampleImages.add(sampleImage(value['filename'],true,'reciever'));});
            return Container(
              child: Column(
                children: <Widget>[
                  Align(
                    alignment: Alignment.centerRight,
                    child: RaisedButton(
                      onPressed: images.length + snapshot.data.length < 5 ? chooseUpload : null,
                      child: Text("Upload Image"),
                    ),
                  ),
                  Container(
  ...
  loadFeedbackRequest() async{
    ...
    Map itemDetail = await DataController.getItemDetails(bidder.suggestedItemId);
    if(bidder.suggestedItemId != widget.itemDetail['id']) setTextField(itemDetail[0]);
  }
  ...
  itemImages(Map images){
    ...
    if(images.isNotEmpty)images.forEach((key, value) { listImages.add(sampleImage(value['filename'],true));});
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      child: Container(
        height: MediaQuery.of(context).size.height*.2,
        child: Row(
          children: images.isEmpty ? [Center(child: Text("No Sample Image"))] : listImages,
        ),
      )
    );
  }
            ...
            return Container(
              child: Column(
                ...
                children: [
                  ...
                  Text('Item Description : '+widget.itemDetail['description']),
                  Text('Budget Price : P'+widget.itemDetail['itemPriceorBudget']),
                  SizedBox(height: 7),
                  Text('Request by : '+widget.userDetail['firstname']+' '+widget.userDetail['lastname']),
  ...
  List<Widget> buttons(){
    ...
    if(widget.otherBidItem['requestStatus'] == '12'){
      ...
      buttons.add(RaisedButton( 
          color: Colors.blue,
          onPressed: (){
            heightMultiplier != .7 ? heightMultiplier = .7 :validateAcceptUpdate();
            refreshState();
          }, 
          child: Text("Accept")
        )
      ...
      title: Container(
        child: Row(
          children: [
            ...
            widget.itemDetail['id'] != bidder.suggestedItemId ? RaisedButton( 
              color: Colors.blue,
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: <Widget>[
              ...
              heightMultiplier == .7 ? Text('Your Feedback Request:') : Container(),
              heightMultiplier == .7 ? form("Item Name", 0, TextInputType.text) : Container(),
              heightMultiplier == .7 ? form("Item Condition", 1, TextInputType.text) : Container(),
              heightMultiplier == .7 ? form("Item Price", 2, TextInputType.number) : Container(),
              heightMultiplier == .7 ? sampleItemsImage(widget.otherBidItem['suggestedItemId']): Container(),
              heightMultiplier == .7 ? Divider() : Container(),
            ],
          ),
        )
...
class _BiddingState extends State<Bidding> {
  ...
  imageContainer(filename){
    return Container(
      ...
      child: Image.network(ImageController.getItemNetImage(filename),fit: BoxFit.contain,),
    );
  }
  ...
  Future<Null> itemDetail(item,images,userImage){
    return showDialog(
      context: context,
      barrierDismissible: true,
      builder: (context) => BiddersItemDetail(item: item,images: images,userImage: userImage,),
    );
  }
        ...
        if(data.connectionState ==  ConnectionState.done){
          if(data.hasData){
            if(data.data.isNotEmpty){
              images = [];
              data.data.forEach((key,value) {images.add(imageContainer(value['filename']));});
              return Container(
            ...
            return Container(
              ...
              child: Center(child: Text('No Sample Image',style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15)),)
            );
          }
          return Container(child: SizedBox(height: MediaQuery.of(context).size.height*.1,));
        } 
        return Container(child: SizedBox(height: MediaQuery.of(context).size.height*.1,));
      }
    );
  ...
  Widget bidItemDetail(id){
    return FutureBuilder(
      future: DataController.getItemDetails(id),
      builder: (_,result){
                ...
                children: <Widget>[
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        Text("Item Name: "+result.data[0]['itemName'],style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15),),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: <Widget>[
                        Text("Budget Money: P "+result.data[0]['itemPriceorBudget'],style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15),),
                        Text("Status: Open")
                      ],
                    ),
                  )
        ...
        return Container(child: Center(child: Text('Loading Details ...'),),);
      },
    );
  }
        ...
        builder: (_,result){
          if(result.connectionState == ConnectionState.done){
            if(result.hasData && result.data.isNotEmpty){
              if(result.data[0] != 'NoConnection') return Container(
                height: MediaQuery.of(context).size.height,
                child: ListView.builder(
                  controller: _sc,
                  itemCount: result.data.length,
                  itemBuilder: (_,index){
                    return bidders(result.data[index]['accountId'],result.data[index]['suggestedItemId'],result.data[index]['bidItemId']);
                  }
                ),
              );
              return Center(child: Align(alignment: Alignment.topCenter,child: Text('No Connectivity'),),);
            }
            return Center(child: Align(alignment: Alignment.topCenter,child: Text('No Participants Yet'),),);
          }
          return Center(child: Align(alignment: Alignment.topCenter,child: Text('No Participants Yet'),),);
        },
      ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          Divider(),
          bidItemDetail(_bidItem.bidItem),
          Divider(),
...
class BiddersItemDetail extends StatefulWidget {
  ...
  BiddersItemDetail({Key key,this.item,this.images,this.userImage}):super(key: key);
  ...
  userImage(){
    return Center(
      child: CircleAvatar(
        ...
        child: ClipOval(
          child: widget.userImage.isNotEmpty ? Image.network(ImageController.getUserNetImage(widget.userImage[0]['filename']),fit: BoxFit.cover,width:70,) : Icon(FontAwesome.user_circle_o,color: Colors.white,size: 60,),
        ) 
      ),
    );
  }
  ...
  textContainer(key,value){
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          key != 'Price' ? Text(key+' : ',style: TextStyle(fontWeight: FontWeight.w300,)) :Text(key+' : ',style: TextStyle(fontWeight: FontWeight.w300,)),
          Text(value),
        ],
      ),
    );
